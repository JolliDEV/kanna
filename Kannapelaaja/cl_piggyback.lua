	...
	if not piggyback.InProgress then
		...
		if closestPlayer then
			...
			if targetSrc ~= -1 then
				...
				TriggerServerEvent("kanna:sync",targetSrc)
				ensureAnimDict(piggyback.personPiggybacking.animDict)
				piggyback.type = "piggybacking"
			else
				drawNativeNotification("~r~Ei ketaan lahella kannettavaksi!")
			end
		else
			drawNativeNotification("~r~Ei ketaan lahella kannettavaksi!")
		end
...
RegisterNetEvent("kanna:syncTarget")
AddEventHandler("kanna:syncTarget", function(targetSrc)
	local playerPed = PlayerPedId()
...
RegisterNetEvent("kanna:cl_stop")
AddEventHandler("kanna:cl_stop", function()
	piggyback.InProgress = false
	ClearPedSecondaryTask(PlayerPedId())
	DetachEntity(PlayerPedId(), true, false)
end)
